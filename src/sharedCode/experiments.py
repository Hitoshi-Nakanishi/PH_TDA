...
def train_test_from_dataset(dataset, test_size=0.2, batch_size=64, wanted_views=None):
    ...
    collate_fn = PersistenceDiagramProviderCollate(dataset, 
                          label_map=label_map, wanted_views=wanted_views)
    ...
    data_test = DataLoader(dataset, batch_size=batch_size,
                           collate_fn=collate_fn, shuffle=False,
...
class UpperDiagonalThresholdedLogTransform:
    ...
    def __call__(self, dgm):
        """
        dgm ... pytorch tensor of persistent homology diagram, initialized by n_elements
        """
        if dgm.ndimension() == 0:
            return dgm
        if dgm.is_cuda:
        ...
        x = torch.mul(dgm, self.b_1.repeat(dgm.size(0), 1)) # (X, 2) * (X, 2) -> (X, 2)
        x = torch.sum(x, 1)
...
class SLayerPHT(Module):
    def __init__(self, n_directions, n_elements, point_dim, n_neighbor_directions=0,
        ...
        self.slayers = [slayer.SLayer(n_elements, point_dim, center_init, sharpness_init)
